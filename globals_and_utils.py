...
UDP_BUFFER_SIZE = int(
    math.pow(2, math.ceil(math.log(IMSIZE * IMSIZE + 1000) / math.log(2)))
)

EVENT_COUNT_CLIP_VALUE = 16  # full count value for colleting histograms of DVS events
SHOW_DVS_OUTPUT = (
    False  # producer shows the accumulated DVS frames as aid for focus and alignment
)
MIN_PRODUCER_FRAME_INTERVAL_MS = (
    5.0  # inference takes about 3ms and normalization takes 1ms, hence at least 2ms
)
...
MAX_SHOWN_DVS_FRAME_RATE_HZ = (
    15  # limits cv2 rendering of DVS frames to reduce loop latency for the producer
)
FULLSCREEN = True  # for kiosk demo autostart
FULLSCREEN_ASPECT_RATIO = 16.0 / 10  # set according to laptop screen
...
MODEL_BASE_NAME = (
    "model_185"  # base name of checkpoint .index and .data files in the MODEL_DIR
)
DEXTRA_NET_BASE_NAME = "dextra_roshambo"  # base name
TFLITE_FILE_NAME = (
    DEXTRA_NET_BASE_NAME + ".tflite"
)  # tflite model is stored in same folder as full-blown TF2 model
SYMBOL_TO_PRED_DICT = {
    "background": 3,
    "paper": 0,
    "scissors": 1,
    "rock": 2,
}  # maps from symbol name to prediction number from CNN
PRED_TO_SYMBOL_DICT = {
    v: k for k, v in SYMBOL_TO_PRED_DICT.items()
}  # maps from prediction number to symbol name
...
# museum settings
MUSEUM_OPENING_TIME = datetime.strptime(
    "11:50", "%H:%M"
).time()  # start when attracting movements are shown
MUSEUM_CLOSING_TIME = datetime.strptime("17:15", "%H:%M").time()
MUSEUM_DEMO_MOVEMENT_INTERVAL_M = 90  # minutes between showing attracting movements of RSP movement if no cmd has been sent
...
MUSEUM_ACTIONS_LOGGING_INTERVAL_MINUTES = (
    10  # minutes between logging number of hand actions
)
MUSEUM_ACTIONS_CSV_LOG_FILE_CREATION_INTERVAL_HOURS = (
    24  # how many hours between creating new activity CSV file
)
MUSEUM_I_AM_ALIVE_LOG_INTERVAL_MINUTES = (
    10  # how many minutes between logging "I'm alive" messages
)
...
MUSEUM_SCREEN_DIM_NO_ACTIONS_TIMEOUT_S = (
    15  # time to dim screen if no actions for this many seconds
)
MUSEUM_DIMMED_SCREEN_BRIGHTNESS = 0.03  # how much to dim to, 0-1 range
MUSEUM_ACTIONS_DIR = "actions"  # where the actions csv files are put

# saving frames
SAVE_FRAMES_INTERVAL = 10
SAVE_FRAMES_STORAGE_LOCATION = (
    "frames"  # saved in logging folder too, in this subfolder
)
...
def input_with_timeout(prompt, timeout=30):
    ...
    try:
        time.sleep(0.5)  # get input to be printed after logging
        return input(prompt)
    ...
    while "the answer is invalid":
        try:
            ...
            reply = (
                str(
                    input_with_timeout(
                        f"{question} {to_str} ({y}/{n}): ", timeout=timeout
                    )
                )
                .lower()
                .strip()
