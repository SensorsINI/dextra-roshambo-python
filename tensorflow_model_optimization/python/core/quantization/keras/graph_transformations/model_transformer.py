...
class ModelTransformer(object):
    ...
    def transform(self):
        ...
        for layer in self.model.layers:
            try:
                check_exist = self._layer_weights_map[layer.name]
                print("Error, key already exist in dictionary: {}".format(layer.name))
            except KeyError:
                pass
        ...
        while True:
            ...
            for transform in self.transforms:
                ...
                while True:
                    match_layer_node = self._find_pattern(transform.pattern(), self._get_matched_layers(transform))
        ...
        if self._is_functional_model(self.model):
            ...
            transformed_model = keras.Sequential.from_config(self._config, custom_objects)

        for layer_index, layer in enumerate(transformed_model.layers):
            if type(layer) == tf.keras.Model:
                original_model = self.model.get_layer(layer.name)
                print("Applying custom cloning to model {}".format(original_model.name))
                transformed_model.layers[layer_index].set_weights(original_model.get_weights()) # clone_model is dumb enough to not copy the weights
            else:
