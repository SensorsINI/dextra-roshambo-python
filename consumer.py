...
class majority_vote:
    ...
    def __init__(
        self, window_length, num_classes
    ):  # window_length is size of window in votes, num_classes is the number of possible values, 0 to num_classes-1
        ...
        self.cirbuf = np.full(
            self.window_length, -1, dtype=np.int8
        )  # cirular buffer of most recent predictions
        self.cmdcnts = np.zeros(
            num_classes, dtype=np.int8
        )  # hold the number of votes for each prediction
        self.num_predictions = 0

    def new_prediction_and_vote(
        self, symbol
    ):  # cmd is the new value, in range 0 to num_classes-1
        ...
        if 0 <= symbol < self.num_classes:
            ...
            if self.num_predictions > self.window_length:
                self.cmdcnts[
                    self.cirbuf[idx]
                ] -= 1  # decrement count for previous prediction but only if we already filled the buffer, otherwise we end up with negative background
...
def classify_img(img: np.array, interpreter, input_details, output_details):
    ...
    interpreter.set_tensor(
        ...
        (1 / 256.0)
        * np.array(np.reshape(img, [1, IMSIZE, IMSIZE, 1]), dtype=np.float32),
    )
    interpreter.invoke()
    pred_vector = interpreter.get_tensor(output_details[0]["index"])[0]
    pred_idx = np.argmax(np.array(pred_vector))
    pred_class_name = list(SYMBOL_TO_PRED_DICT.keys())[
        list(SYMBOL_TO_PRED_DICT.values()).index(pred_idx)
    ]
...
def consumer(queue: Queue):
    ...
    def log_i_am_alive_message():
        ...
        now = time.time()
        if (
            now - museum_last_i_am_alive_log_time_sec
            > MUSEUM_I_AM_ALIVE_LOG_INTERVAL_MINUTES * 60
        ):
    ...
    def send_cmd(cmd, update_time_sent=True):
        ...
        try:
            if serial_port_instance is None:
                serial_port_instance = open_serial_port(
                    serial_port_name
                )  # try opening it if it does not exist, maybe got replugged or lost power
    ...
    def maybe_show_demo_sequence():
        ...
        if time_now > MUSEUM_OPENING_TIME and time_now < MUSEUM_CLOSING_TIME:
            log.debug(
                f"showing demo movement because {MUSEUM_DEMO_MOVEMENT_INTERVAL_M} minutes since last demo movement"
            )
            show_demo_sequence()
        else:
            log.debug(
                f"not showing demo sequence because we are outside museum opening hours {MUSEUM_OPENING_TIME} to {MUSEUM_CLOSING_TIME}"
            )
    ...
    def show_demo_sequence():
        ...
        try:
            for c in cmds:
                send_cmd(
                    c, update_time_sent=False
                )  # dont update time so we don't light up backlight for these movements
                time.sleep(interval_seconds)

        except serial.serialutil.SerialException as e:
            log.error(
                f"Error writing to serial port {SERIAL_PORT} with cmd {cmd} for detected symbol {pred_name}: {e}"
            )
    ...
    def create_museum_actions_logging_csv_file() -> str:
        ...
        museum_csv_actions_logging_file_name = os.path.join(
            museum_actions_dir,
            MUSEUM_LOGGING_FILE
            + "-"
            + platform.node()
        ...
        with open(
            museum_csv_actions_logging_file_name, "w", newline=""
        ) as museum_csv_logging_file:
            museum_csv_writer = csv.writer(museum_csv_logging_file, dialect="excel")
            museum_csv_writer.writerow(
                [
                    "year",
                    "day_of_year",
                    "weekday",
    ...
    def write_actions_to_csv():
        ...
        now = datetime.now()
        minutes_since_last = (
            int(time.time()) - museum_last_time_movements_written_sec
        ) / 60
        year = now.year
        weekday = now.weekday()
        ...
        try:
            with open(
                museum_csv_actions_logging_file_name, "a", newline=""
            ) as museum_csv_logging_file:
                museum_csv_writer = csv.writer(museum_csv_logging_file, dialect="excel")
                museum_csv_writer.writerow(
                    [
                        ...
                        minute,
                        minutes_since_last,
                        museum_movements_since_last_log,
                    ]
                )
        except Exception as e:
            log.error(
                f"could not write action count to {museum_csv_actions_logging_file_name}: {e}"
            )
        museum_movements_since_last_log = 0
        museum_last_time_movements_written_sec = int(time.time())

    parser = argparse.ArgumentParser(
        description="consumer: Consumes DVS frames for trixy to process",
        allow_abbrev=True,
    ...
    cv2.putText(
        img, "x: exit", (1, 10), cv2.FONT_HERSHEY_PLAIN, 0.6, (255, 255, 255), 1
    )
    cv2.putText(
        img, "space: move", (1, 30), cv2.FONT_HERSHEY_PLAIN, 0.6, (255, 255, 255), 1
    )
    ...
    if not SAVE_FRAMES_STORAGE_LOCATION is None and SAVE_FRAMES_INTERVAL > 0:
        ...
        if not os.path.exists(save_frames_folder):
            log.info(
                f"creating folders to hold sample frames that will be stored every {SAVE_FRAMES_INTERVAL} new classifications"
            )
    ...
    log.info(
        f"scheduling 'I am alive' logging every MUSEUM_I_AM_ALIVE_LOG_INTERVAL_MINUTES={MUSEUM_I_AM_ALIVE_LOG_INTERVAL_MINUTES}m"
    )
    schedule.every(MUSEUM_I_AM_ALIVE_LOG_INTERVAL_MINUTES).minutes.do(
        log_i_am_alive_message
    )
    ...
    log.info(
        f"scheduling hand action count every MUSEUM_ACTIONS_LOGGING_INTERVAL_MINUTES={MUSEUM_ACTIONS_LOGGING_INTERVAL_MINUTES}m"
    )
    schedule.every(MUSEUM_ACTIONS_LOGGING_INTERVAL_MINUTES).minutes.do(
        write_actions_to_csv
    )
    log.info(
        f"scheduling attracting demo movement every MUSEUM_HAND_MOVEMENT_INTERVAL_M={MUSEUM_DEMO_MOVEMENT_INTERVAL_M}m"
    )
    ...
    log.info(
        "starting main consumer loop; in display, hit x to exit or spacebar to show demo movement"
    )
    try:
        while True:
            ...
            if (
                dt := time.time() - time_last_sent_new_cmd
                > MUSEUM_SCREEN_DIM_NO_ACTIONS_TIMEOUT_S
            ):
            ...
            with Timer("overall consumer loop", numpy_file=None, show_hist=False):
                ...
                with Timer(
                    ...
                    pred_name, pred_idx, pred_vector = classify_img(
                        img, interpreter, input_details, output_details
                    )

                if pred_idx <= 3:  # symbol recognized (or background==3)
                    cmd = pred_to_cmd_dict[
                        pred_idx
                    ]  # start with no command sent to hand
                    if cmd_voter:
                    ...
                    if (
                        not save_frames_disabled
                        and SAVE_FRAMES_INTERVAL > 0
                        and save_frames_folder
                        and pred_name != last_prediction_name
                        and frame_number - save_frames_last_frame_saved
                        >= SAVE_FRAMES_INTERVAL
                    ):
                        fname = str(int(time.time())) + ".png"
                        path = os.path.join(save_frames_folder, pred_name, fname)
                        log.debug(
                            f"saving new predicted {pred_name} frame # {frame_number} as file {path}"
                        )
                        ...
                        if (
                            frame_number % 1000 == 0
                        ):  # only check disk every thousand frames
                            free_gb = shutil.disk_usage(".").free / 1.0e9
                            if free_gb < SAVE_FRAMES_DISK_FREE_STOP_LIMIT_GB:
                                log.warning(
                                    f"saving frames disabled because free_gb={free_gb:.1f} is less than < SAVE_FRAMES_DISK_FREE_STOP_LIMIT_GB={SAVE_FRAMES_DISK_FREE_STOP_LIMIT_GB}"
                                )
                    ...
                    if (
                        cmd
                        and time.time() - time_last_sent_new_cmd
                        > MIN_INTERVAL_S_BETWEEN_CMDS
                    ):
                        log.debug(
                            f"sending cmd {cmd} for pred_idx {pred_idx} and detected symbol {pred_name}"
                        )
                        send_cmd(cmd)
                ...
                with Timer(
                    "producer->consumer inference delay", delay=dt, show_hist=False
                ):
                    pass
    finally:
...
def open_serial_port(serial_port_name):
    ...
    if not serial_port_name is None:
        log.debug(
            "opening serial port {} to send commands to finger".format(serial_port_name)
        )
        try:
